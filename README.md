...
**路由器的定义：**
路由器在网路层实现网间互联，它可以实现同类型或不同类型的网络互连，包括LAN与LAN，LAN与WAN，WAN与WAN的互连。是一个具有多个输入端口和多个输出端口的专用计算机，其功能是转发分组，它把从某一输入端口收到的分组按照分组所携带的目的地址从适当的输出端口转发出去。
...
1. 端口转发(映射)：
    ...
    问：如果一个WAN口的主机，需要**主动**连接LAN口的主机IP:Port，（例如：想在LAN口架设一台服务器，WAN口要请求服务器），怎么能够实现呢？
